...
const SignUP = ({ api , allProfiles}) => {
  ...
  const [referralMessage, setReferralMessage] = useState(true)
  ...
  const sortedProfiles = [...allProfiles].sort((a, b) => a.country.localeCompare(b.country));


  useEffect(()=>{
if (referralCode.length === 0){
  if (referralCode && !allProfiles.find((user)=>user.referralCode ===referralCode)){
    setReferralMessage(false);
  }
  else{
    setReferralMessage(true)
  }
 setReferralMessage(nul)
}
      ...
      <div className="signup-card">
        ...
        <form onSubmit={handlesubmit}>
          <div className="signup-card-body">
            ...
            <div className="form-group">
              ...
              <select value={selectedValue} onChange={handleSelectChange}>
                {sortedProfiles.map((profiles, index) =>(
                  <option key={index} value={profiles.country}>{[profiles.country].sort()}</option>
                ))}
            ...
            <div className="form-group">
              <label>Referral Code  {referralMessage === false ? (<label style={{color:"red"}}>Invalid Referal Code</label>):(<label style={{color:"green"}}>Valid Referral</label>)} </label>
              {referralCode.length >=1000 ? ( <input
                value={referralCode}
                type="text"
               readOnly
              />):(
