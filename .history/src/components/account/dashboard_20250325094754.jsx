...
const Dashboard = ({ api, user, error }) => {
      ...
      <div style={styles.mainContent}>
        <Routes>
          ...
          <Route
            path="/dproducts"
            element={
              <Products products={products} error={error}  user={user} handleDelete={handleDelete} />
            }
          />
           <Route
...
const Products = ({ products, api, handleDelete }) => {
  const { t } = useTranslation();
const [ownersproducts,setProducts]= useState(products)
    const owner = 9
   
          const products = data.filter((product) => product.ownerId === owner);
...
const Orders = ({ orders }) => {
  ...
  return (
    <div>
      <h1 style={sharedStyles.header}>{t("Orders Management")}</h1>
      <div style={sharedStyles.grid}>
        {orders.map((order) => (
