...
function SignIn({ api, handleLogin, add }) {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      login(data.token, data.role, data.user.userName,data.user.profileImage, data.user, user);
      localStorage.setItem("userId", data.user.id);
      dispatch(loadCartAfterLogin(data.user.id));
    localStorage.setItem("userData", dauser);

      handleWishlistmerge(data.user.id);
      navigate("/dashboard");
      console.log("hushme",data.user.id)
      // console.log("hushme",data.id)
