...
{groupedBrandsByCategory[cat]?.map((brandArray, idx) => {
  const brandNames = Array.isArray(brandArray)
    ? [...new Set(brandArray.map(b => b.name))].join(", ")
    : "Invalid brand data";

  return (
    <h5 key={idx} onClick={() => handleClick(brandArray)}>
      {brandNames}
    </h5>
  );
})}
