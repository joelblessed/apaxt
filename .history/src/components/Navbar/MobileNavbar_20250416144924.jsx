...
{groupedBrandsByCategory[cat]?.map((brandGroup, idx) => {
  const brandNames = Array.isArray(brandGroup)
    ? [...new Set(brandGroup.map(b => b.name))].join(", ")
    : "No brands available";

  return (
    <h5 key={idx} onClick={() => handleClick(brandGroup)}>
      {brandNames}
    </h5>
  );
})}
