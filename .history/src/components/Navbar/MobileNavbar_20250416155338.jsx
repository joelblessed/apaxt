...
const [catName, setCatName] 
  ...
  const brandShow = (event) => {
    setIsBrandOpen((prevShow) => !prevShow);
    if (previewRef.current && !previewRef.current.contains(event.target)) {
    }
  };
        ...
        <li style={{ listStyleType: "none", padding: 0 }}>
          ...
          {categories.map((cat, index) => (
           ...
           <div> {isOpen && (
              ...
              <div>{isOpen===true &&(<button onClick={() => {setCategoryName(cat); brandShow()}}>Brand</button>)} </div>
           </div>
              {isBrandOpen && cat ===catName && [
                ...new Map(
                ...
                <h6 key={idx} onClick={() => handleClick(brand.name)}>
                  {brand.name}
                </h6>
              ))}
            </div>
