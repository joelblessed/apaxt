...
function DesktopNavbar({
  ...
  return (
    <Navbar>
      ...
      <NavList gap="20px">
        <NavItem>
          <NavLinkStyled onClick={search} to="/" exact>
          {t("Home")}
          </NavLinkStyled>
        </NavItem>
        <NavItem>
          <NavLinkStyled onClick={search} to="/products">{t("Products")}</NavLinkStyled>
        </NavItem>
        <NavItem>
          <NavLinkStyled  to="/formUpload">{t("Sell")}</NavLinkStyled>
        </NavItem>
        <NavItem>
          <NavLinkStyled onClick={search} to="/category"></NavLinkStyled>
          <p>Categories:</p>
      <ul   onClick={() => {setSearchTerm(category); search()} style={{ listStyleType: "none", padding: 0 }}>
        {categories.map((category, index) => (
          <li
            key={index}
            onClick={() => {setSearchTerm(category); search()}}
            style={{
              cursor: "pointer",
              marginBottom: "5px",
              padding: "5px",
          ...
          >
            <h4 style={{ color: "red", margin: 0 }}>{category}</h4>
          </li>
        ))}
      </ul>
            {/* <StyledSelect
              ...
              onChange={(e) => setSearchTerm(e.target.value)}
            >
              <StyledOption  value="">{t("Category")}</StyledOption>
              {categories.map((category, index) => (
