...
const Home = ({
  ...
  const fetchViewedProducts = useCallback(async (userId) => {
    try {
      const response = await fetch(`${api}/viewedProducts/${userId}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch viewed products: ${response.statusText}`);
      }
      const data = await response.json();
      console.log("Viewed products:", data);
      return data.map((viewed) => viewed.productId);
    } catch (error) {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const viewedProductIds = await fetchViewedProducts(userId);
      const filtered = glofilteredProducts.filter((product) =>
        viewedProductIds.includes(product.id)
      );
      console.log("Filtered viewed products:", filtered);

      setViewed(filtered);
    };

    getData();
  }, [fetchViewedProducts, glofilteredProducts, userId]);
