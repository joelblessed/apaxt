...
console.log(category)
  ...
  useEffect(() => {
    if (ownerName || category || (searchTerm.trim() !== "" && searchTerm)) {
      ...
      if (searchTerm.length === 0) {
        ...
        const results = fuse.search(cate.trim());
  ...
  return (
    <div style={{display:"flex"}}>
      <div style={{marginTop:"80px", marginLeft:"30px", marginRight:"-70px"}}>
      <h4>{ownerName}'s Products</h4>

        {uniqueCategories.map((categoryName, index)=>(
          <label onClick={ () => setCategory(categoryName)} style={{
            cursor: "pointer",
