...
import CategoryBox from "./categoryBox"; // Import CategoryBox component
  ...
  const groupByBrand = useCallback((products) => {
    return products.map((product) => product.brand.map((brand)=> brand.reduce((acc, brand) => {
  ...
  const bDobject = useMemo(
    () => Object.keys(groupedBrand),
    [groupedBrand]
  );
  ...
  const bDobject1 = useMemo(() => {
    return Dobject.reduce((acc, brand) => {
      acc[category] = groupedBrand[category].slice(0, 4);
      return acc;
    }, {});
  }, [Dobject, groupedProducts]);
  return (
