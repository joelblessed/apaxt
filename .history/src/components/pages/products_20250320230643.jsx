...
const Products = ({
  ...
  useEffect(() => {
    if (productId !== null) {
      const fetchProduct = async () => {
        ...
        setLiked(product.likedBy.some(user => user.userId === userId)); // Check if the user has liked this product
      };
  ...
  const toggleLike = async () => {
    ...
    const response = await fetch(`${api}/products/${productId}/${endpoint}`, {
      ...
      body: JSON.stringify({ username,userId }), // Send userId to backend
    });
    ...
    <>
      <div className="animated-box" style={styles.container}>
        {filteredProducts.length > 0 ? (
          ...
          <button onClick={() => toggleLike(selectedProduct); setSelectedProduct(product)}>
