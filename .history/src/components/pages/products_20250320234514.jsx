...
const Products = ({
  ...
  const dispatch = useDispatch();// Function to check screen size
    ...
    const [selectedProduct, setSelectedProduct] = useState([ {likedBy:[]}]);
  ...
  const toggleSelection = (id, currentSelection) => {
   ...
   // Fetch all products on component mount and auto-select the first product if available
   useEffect(() => {
  ...
  // Toggle like/dislike for the selected product using a single button
  const toggleLike = async () => {
    if (!selectedProduct) return; // Do nothing if no product is selected

    const liked = selectedProduct.likedBy.some(user => user.userId === userId);
    const endpoint = liked ? "dislike" : "like";

    const response = await fetch(${api}/products/${selectedProduct.id}/${endpoint}, {
      method: "PATCH",
    ...
    // Update products array and selectedProduct with the new data from the backend
