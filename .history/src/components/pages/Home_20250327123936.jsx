...
const Home = () => {
  ...
  const mockBlogs = [
    {
      ...
      summary: "Latest updates in e-commerce.",
    },
  ];
  ...
  useEffect(() => {
    const storedLastViewed =
      JSON.parse(localStorage.getItem("lastViewed")) || [];
  ...
  const handleProductClick = (product) => {
    const updatedLastViewed = [
      product,
      ...lastViewed.filter((p) => p.id !== product.id),
    ].slice(0, 10);
      ...
      <BlogWrapper>
        {mockBlogs.map((blog, index) => (
          <BlogCard key={index}>
            <h3>{blog.title}</h3>
            <p>{blog.summary}</p>
            <BlogButton onClick={() => navigate(`/blog/${blog.id}`)}>
              Read More
            </BlogButton>
          </BlogCard>
        ))}
