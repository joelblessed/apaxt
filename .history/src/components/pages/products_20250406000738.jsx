...
const Products = ({
  ...
  const [isMobile, setIsMobile] = useState(window.innerWidth < 1);
...
useEffect(() => {
  ...
  useEffect(() => {
    if (isMobile) {
      mobileSearchHandler(searchTerm);
    } else {
  ...
  }, [searchTerm, isMobile, glofilteredProducts]);
  ...
  const mobileSearchHandler = async (searchTerm) => {
    console.log("Running mobile search for:", searchTerm);
  
    if (!searchTerm || searchTerm.trim() === "") {
      setFilteredProducts(products); // fallback to all
      return;
    }
