...
const Home = ({ filteredProducts, mobilefilteredProducts,api, searchTerm, highlightText,SelectedProduct, addToWishList, addToCart}) => {
  ...
  const [currentIndex, setCurrentIndex] = useState(0);
    ...
    const [filteredCategory, setFilteredCategory]= useState(n)
    ...
    const handleFilteredCategory = (category =>{
      const filterCategory = filteredProducts.filter((product) => product.category === category);
      ...
      {Object.keys(groupedProducts).map((category, index) => (
        <div key={index} style={styles.categoryContainer}>
          ...
          <div style={styles.productsGrid}>
            ...
            <div>
            <Link to={`/category/${filteredCategory}`} onClick={()=>{handleFilteredCategory(category);}} style={styles.viewMoreButton}>
            {t("View More")}
          </Link>
            </div>
          </div>
