...
const Category = ({
  ...
  // Handle search functionality
   ...
   useEffect(() => {
      if (selectedCategory || (searchTerm.trim() !== "" && searchTerm)) {
        const fuse = new Fuse(fetchProducts, {
          keys: ["name", "category", "owner", "brand.name"],
          threshold: 0.3,
        });
  
        if (searchTerm.length === 0) {
          const results = fuse.search(selectedCategory.trim());
        ...
        setProducts([]); // Reset to owner's products
        setHasMore(true); // Enable pagination again
      }
    }, [searchTerm, selectedCategory]);
  
  // useEffect(() => {
  //   if (selectedCategory || casearchTerm && searchTerm.trim() !== "") {
  //     const results = fuse.search(searchTerm.trim());
  //     const matched = results.map((res) => res.item);
  //     setProducts(matched);
  //     setHasMore(false);
