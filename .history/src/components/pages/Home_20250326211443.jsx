...
const Home = ({brands, search, setSearchTerm}) => {
      ...
      <div style={{ overflow: "hidden", width: "1300px" ,background:"" }}>
        ...
        >
          {brands.map((box, index) => (
            ...
            }}>onClick={()=>{setSearchTerm{box}}}
              <label style={{fontWeight:"bold", fontSize:"50px", color:"white"}} onClick={()=>{setSearchTerm(box)}}>{box}</label>
            </div>
          ))}
        </div>
      </div>
