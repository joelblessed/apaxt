...
const Home = ({brands}) => {
  ...
  return (
    <div style={{ textAlign: "center",background:"", marginTop: "20px", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px" }}>
      ...
      {startIndex > 0 && (
        <button onClick={prevBoxes} style={{ padding: "10px", cursor: "pointer", background:"none", border:"none" }}>less</button>
      )}
        ...
        >
          {brands.map((box, index) => (
            <div key={index} style={{
              ...
              <label style={{fontWeight:"bold", fontSize:"50px", color:"white"}}>{box}</label>
            </div>
          ))}
        </div>
      </div>
      ...
      {startIndex + boxesPerPage < boxes.length && (
        <button onClick={nextBoxes} style={{ padding: "10px", cursor: "pointer", background:"blue", background:"none", border:"none"  }}>more</button>
      )}
