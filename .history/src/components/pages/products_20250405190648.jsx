...
const Products = ({
  ...
  const fetchProducts = async () => {
    const res = await fetch('http://localhost:5000/products'); // Adjust API endpoint
    const data = await res.json();
    setProducts(data);
  ...
  const debouncedSearch = debounce((query) => {
    ...
    // Create a Fuse.js instance with the product data
    const fuse = new Fuse(products, {
      keys: ['name', 'category', 'owner', 'brand.name'], // List the keys you want to search
      threshold: 0.4, // Adjust the threshold (lower is stricter, higher is looser)
    });

    // Perform the search using Fuse.js
    const result = fuse.search(query).map((result) => result.item); // Extract items from the results
  ...
  // Call fetchProducts on initial render
  useEffect(() => {
    fetchProducts();
  }, []);
