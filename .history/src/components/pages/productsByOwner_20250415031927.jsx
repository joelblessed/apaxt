...
const ProductsByOwner = ({
  ...
  const fetchSearchResults = useCallback(async (query) => {
    try {
      const res = await fetch(`${api}/search?query=${encodeURIComponent(query)}&page=${page}&limit=5`);
      const data = await res.json();
  ...
  return (
    <div>
      {products.length > 0 ? (
        <div>
          <h4 style={{textAlign:"center", marginTop:"40px"}}>{ownerName} Products</h4>
          <Box
            Mobject={products}
            Dobject={products}
            loaderRef={loaderRef}
        ...
        <dh4iv style={{textAlign:"center", marginTop:"40px"}}> {ownerName} dosen't have the  products Found </h4>
      )}

      {generalProducts.length > 0 && (
        <div>
          <h4  style={{textAlign:"center", marginTop:"40px"}}>Product other Sellers</h4>
          <Box
