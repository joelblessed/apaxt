...
const Payment = ({api,amount,checkout,setPaymentStatus,setPhoneNumber,paymentRefId,setPaymentId}) => {
    ...
    async function createApiUser() {
        ...
        const response = await fetch(`${api}/create-api-user`, {
            method: 'POST',
    ...
    async function getCreatedUser(userId) {
        ...
        const response = await fetch(`${api}/get-created-user/${userId}`);
        return response.json();
    }
    
    async function retrieveApiKey(userId) {
        console.log('Retrieving API key...');
        const response = await fetch(`${api}/retrieve-api-key/${userId}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        return response.json();
