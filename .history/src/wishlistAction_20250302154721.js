...
export const addToWishListBeforeLogin = (product) => (dispatch, getState) => {
  ...
  localStorage.setItem("WishList", JSON.stringify(getState().wish.items));
};
...
export const loadWishListAfterLogin = () => async (dispatch) => {
  ...
  try {
    // Fetch user wish from the server
    const response = await fetch(API_URL, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });

    let userCart = response.ok ? await response.json() : { wish: [] };

    // Merge Local Cart & Server Cart (Remove Duplicates)
    const mergedCart = [
      ...new Map(
        [...localCart, ...userCart.wish].map((item) => [item.id, item])
      ).values(),
    ];
    ...
    // Save merged wish to the server
    ...
    // Clear local wish after merging
...
export const addToWishListAPI = (product) => async (dispatch) => {
  const token = localStorage.getItem("token");
