...
const filteredProductsBySeller =
  brand === "All"
    ? products
    : products.filter((product) => 2 === product.ownerId);
    ...
    useEffect(() => {
      fetch(`${api}/products`)
        ...
        .then((res) => res.json())
        .then((data) => {
          const filteredProducts = data.filter((product) => product.ownerId === userId);
  ...
  const handleSearch = () => {
    ...
    let filtered = products.filter((product) => {
      ...
      const ownerNameMatch = product.name.toLowerCase().includes(query);
    ...
    <AuthProvider>
      ...
      <Router>
        ...
        <Routes>
          <Route
            ...
            element={
              <Home
                ...
                addToCart={addToCart}
              filteredProductsBySeller={filteredProductsBySeller}
          ...
          <Route
            path="/orders"
            element={<Orders token={token} userId={userId} api={api} allProfiles={allProfiles} />}
          />
           <Route
            path="/orders2"
            element={<Orders2 token={token} userId={userId} api={api} allProfiles={allProfiles} />}
          />
          <Route
            path="/profile"
          ...
          <Route
            ...
            element={
                ...
                <Dashboard api={api} user={user} error={error} ownersProducts={ownersProducts} />
