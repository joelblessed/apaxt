  ...
  useEffect(() => {
    fetch(`${api}/products`)
      ...
      .then((data) => {
        ...
        // Extract unique discount values greater than 0
        const uniqueDiscounts = [...new Set(productsData
          .filter(product => product.discount > 0) // Keep only products with discounts
          .map(product => product.discount) // Extract discount values
        )];
        
        setDiscount(uniqueDiscounts);
