...
export const addToWishListBeforeLogin = (product) => (dispatch, getState) => {
  ...
  localStorage.setItem("WishList", JSON.stringify(getState().wishlist.items));
};
...
export const loadWishListAfterLogin = () => async (dispatch) => {
  ...
  try {
    // Fetch user wishlist from the server
    const response = await fetch(API_URL, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });

    let userCart = response.ok ? await response.json() : { wishlist: [] };

    // Merge Local Cart & Server Cart (Remove Duplicates)
    const mergedCart = [
      ...new Map(
        [...localCart, ...userCart.wishlist].map((item) => [item.id, item])
      ).values(),
    ];
    ...
    // Save merged wishlist to the server
